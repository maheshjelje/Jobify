...
export const validateRegister = withValidationErrors([
    ...
    body('email').notEmpty().withMessage('email required').isEmail().withMessage('invali email format')
        .custom(async (email) => {
    ...
    body('password').notEmpty().withMessage('passoword required').isLength({ min: 8 })
        .withMessage('Passoword must be 8 characters long'),
    body('lastName').notEmpty().withMessage('lastName required'),
    body('location').notEmpty().withMessage('location required')

])

export const ValidationLogin = withValidationErrors([
    body('email').notEmpty().withMessage('email required').isEmail().withMessage('invali email format'),

    body('password').notEmpty().withMessage('passoword required')
])
